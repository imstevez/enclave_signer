...
pub fn generate_evm_account() -> Result<(Address, Vec<u8>, Vec<u8>)> {
...
pub async fn sign_message(private: &[u8], message: &str) -> Result<Vec<u8>> {
...
pub fn address_checksum(address: &Address) -> String {
    eth_checksum::checksum(&hex::encode(address.as_bytes()))
}
...
async fn test_sign() {
    ...
    let message = "hello";
    let sig = sign_message(&prv_bytes, message).await.unwrap();
